...
GUIPauseButton::GUIPauseButton(si32 width, si32 height)
:GUIButton(width, height)
{
    
}

GUIPauseButton::GUIPauseButton(GUIContainer* parent, si32 width, si32 height)
...
void GUIPauseButton::renderRays( float rayWidth )
{
    int rayCount = 0;
...
sbool GUIPauseButton::OnEvent( Event& event)
{
...
void GUIPauseButton::rayTraceForSinkGlow()
{
    if( !m_bIsClipped )
    {
        return;
    ...
    Vector2f p0( -h*0.4f, -h*0.6f);
    Vector2f p1( -h*0.4f, h*0.6f);
    Vector2f p2( h*0.4f, -h*0.6f);
    Vector2f p3( h*0.4f, h*0.6f);
...
void GUIPauseButton::pushRayDataToRayVertexBuffer(float rayWidth, Ray& ray, int rayCount)
{

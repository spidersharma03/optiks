...
GUIHintButton::GUIHintButton(si32 width, si32 height)
:GUIButton(width, height)
{
    
}

GUIHintButton::GUIHintButton(GUIContainer* parent, si32 width, si32 height)
...
void GUIHintButton::renderRays(float rayWidth)
{
    int rayCount = 0;
...
sbool GUIHintButton::OnEvent( Event& event)
{
...
void GUIHintButton::rayTraceForSinkGlow()
{
    if( !m_bIsClipped )
    {
        return;
    ...
    Vector2f p0( 0.0f, -h*0.4f);
    Vector2f p1( 0.0f, h*0.5f);
    Vector2f p2( 0.0f, -h*0.75f);
    Vector2f p3( 0.0f, -h*0.5f);
...
void GUIHintButton::pushRayDataToRayVertexBuffer(float rayWidth, Ray& ray, int rayCount)
{

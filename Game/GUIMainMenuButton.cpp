...
GUIMainMenuButton::GUIMainMenuButton(si32 width, si32 height)
:GUIButton(width, height)
{
    
}

GUIMainMenuButton::GUIMainMenuButton(GUIContainer* parent, si32 width, si32 height)
:GUIButton(parent, width, height)
{
    ...
    m_pAllRayVertices  = new float[12*2*3];
    m_pAllRayColors    = new float[12*4*3];
    m_pAllRayTexCoords = new float[12*2*3];
}

void GUIMainMenuButton::render()
{
    if( !m_bIsClipped )
    {
        return;
...
void GUIMainMenuButton::renderRays(float rayWidth)
{
    int rayCount = 0;
...
sbool GUIMainMenuButton::OnEvent( Event& event)
{
...
void GUIMainMenuButton::rayTraceForSinkGlow()
{
    if( !m_bIsClipped )
    {
        return;
    ...
    Vector2f p0( -h*0.5f, -h*0.5f);
    Vector2f p1(  h*0.5f, -h*0.5f);
    Vector2f p2( -h*0.5f, 0.0f);
    Vector2f p3( h*0.5f, 0.0f);
    Vector2f p4( -h*0.5f, h*0.5f);
    Vector2f p5( h*0.5f, h*0.5f);
    ...
    p0.set( -h*0.8f, -h*0.5f);
    p1.set(  -h*0.6f, -h*0.5f);
    p2.set( -h*0.8f, 0.0f);
    p3.set( -h*0.6f, 0.0f);
    p4.set( -h*0.8f, h*0.5f);
    p5.set( -h*0.6f, h*0.5f);
...
void GUIMainMenuButton::pushRayDataToRayVertexBuffer(float rayWidth, Ray& ray, int rayCount)
{
